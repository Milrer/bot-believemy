...
export default {
  ...
  async execute(interaction) {
    try {
      ...
      const channelId = interaction.guild.channels.cache.get(process.env.CHANNEL_SEND_ID)

      const rocket = await fetch("https://believemy.com/api/webhooks/check-student", {
        method: "post",
      ...
      if (member.roles.cache.has(studentRoleId)) {
        const studentValited = {
          ...
          color: 0x57F287,
          description: "Vous êtes déjà inscrit. Vous ne pouvez pas vous inscrire à nouveau.",
          footer: {
